...
export const StickyCtaBar: React.FC<StickyCtaBarProps> = ({ onOpenBooking }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (documentHeight > 0) {
        const scrollFraction = window.scrollY / documentHeight;
        setVisible(scrollFraction > 0.4);
      }
    };
    ...
    <div className="hidden lg:flex fixed top-20 left-1/2 -translate-x-1/2 z-30 bg-white/95 backdrop-blur-md px-6 py-2.5 rounded-full border border-slate-200 shadow-xl items-center gap-6 animate-in slide-in-from-top-4 duration-300">
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className="px-4 py-1.5 rounded-full bg-slate-900 hover:bg-slate-800 text-white font-extrabold text-xs flex items-center gap-1.5 shadow-xs transition-all cursor-pointer"
        >
        ...
        <a
          ...
          className="px-3.5 py-1.5 rounded-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs flex items-center gap-1.5 transition-all shadow-xs"
        >
